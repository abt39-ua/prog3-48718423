...
public class RankingPreTest {
	...
	public void testAddScore() {
		...
		for (int i=0; i<10; i++) {
				winsScore.score(1); // 10 veces
				// {"TIEFighter", "TIEBomber", "TIEInterceptor"}
				//   4               3             3
				destroyedScore.score(FighterFactory.createFighter(kREBEL_FIGHTERS[i%3], rebelShip));
		}
		destroyedRanking.addScore(destroyedScore); 
		winsRanking.addScore(winsScore); // 10
		...
		// 4xTIEFighter + 3xTIEBomber + 3xTIEINTERCEPTOR
	...
	public void testGetWinner() {
		//Iniciamos marcadores para Julia
				...
				WinsScore winner = winsRanking.getWinner();
				assertEquals("Player IMPERIAL: 10", winner.toString());
				
				DestroyedFightersScore winnerDestroyed = destroyedRanking.getWinner();
				assertEquals("Player REBEL: 4455", winnerDestroyed.toString());
