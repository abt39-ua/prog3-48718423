...
public class PlayerRandomPreTest {
	...
	public void testInitFightersImperial() {
		...
		compareLines(kIMPERIALGAMESHIP, pr.getGameShip().toString());
	}
	...
	public void testPurgeFleet() {
		...
		List<Fighter> fleet = playerRandom.getGameShip().getFleetTest();
		for(Fighter f : fleet) {
			f.addShield(-1000);
		}
		playerRandom.purgeFleet();
		fleet = playerRandom.getGameShip().getFleetTest();
		assertEquals("despues de la purga", 0, fleet.size());
	}
	...
	/* Realiza el test de comprobación de los parámetros null en PlayerRandom del constructor 
	 y de setBoard */
	//TODO
	@Test
	public void testRequireNonNull()  {
		try {
			PlayerRandom pr = new PlayerRandom(null, 30);
			fail("no compruebas null en el constructor de playerrandom");
		}
		catch(NullPointerException e) {
		}
		try {
			PlayerRandom pr = new PlayerRandom(Side.IMPERIAL, 30);
			pr.setBoard(null);
			fail("no compruebas null en el setBoard");
		}
		catch(NullPointerException e) {
		}
	}
