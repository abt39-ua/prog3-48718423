...
public class ShipPreTestP2 {
	...
	public void testRequireNonNull() throws NoFighterAvailableException {
		try {
			...
			fail("NO lanzas la excepcion nullpointer en el constructor");
		}
		catch(NullPointerException e) {
		}
		Ship ship = null;
		try{
			ship = new Ship("my ship", Side.IMPERIAL);
			ship.addFighters(null);
			fail("NO lanzas la excepcion nullpointer en el addFighters");
		}
		catch(NullPointerException e) {
		}
		try {
			ship.addFighters("3/XWing");
			Fighter f = ship.getFirstAvailableFighter(null);
			fail("NO lanzas la excepcion nullpointer en getFirstAvalaible");
		}
		catch(NullPointerException e) {
